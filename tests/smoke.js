...
assert(appSource.includes("card.classList.add('is-confirming')"), 'task completion micro-interaction missing');
assert(!appSource.includes("card.classList.contains('completed') ? 'is-restoring' : 'is-completing'"), 'task completion still fades the whole card');
assert(!stylesSource.includes('@keyframes taskComplete'), 'task completion still drives a full-card opacity animation');
assert(stylesSource.includes('@keyframes taskCheckConfirm'), 'task check confirmation animation missing');
...
assert(!fluencySource.includes('enriquecida pela IA'), 'the app must not promise an external AI enrichment flow');
...
assert.strictEqual(state.fluency.items.length, 8, 'unauthenticated defaults must contain only generic starter cards');
assert.strictEqual(state.fluency.sources[0].id, 'starter-russian-a1');
assert.strictEqual(context.FluencyEngine.curriculumProgress(state.fluency).lessons.length, 0, 'private lessons must arrive only through authenticated state');
