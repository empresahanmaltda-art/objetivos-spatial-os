...
(async () => {
  ...
  d.querySelector('#notificationSoundTestBtn').click();
  assert(d.querySelector('#notificationSoundTestStatus').textContent.includes('Ative os sons'));
  const settingsBeforeImport = JSON.stringify(api.getState().settings);
  await resources.load(clientFor({ data: [...rows, { kind: 'sound:optional', payload: { ...sounds[0].payload, id:'optional', event:'notification-alternative', label:'Som opcional fictício' } }] }), 'owner-a', { force: true });
  assert.strictEqual(JSON.stringify(api.getState().settings), settingsBeforeImport, 'adding a sound must preserve chosen assignments, volume and mute');
  d.querySelector('#settingsBtn').click();
  assert.strictEqual(d.querySelector('#sound-notification').options.length, 10);
  assert.strictEqual(d.querySelector('#sound-notification').value, 'open');
  assert(d.querySelector('.sound-settings').textContent.includes('iPhone bloqueado'));
  ...
  assert.strictEqual(await engine.unlock(), true, 'explicit gesture unlock must resolve after resume');
