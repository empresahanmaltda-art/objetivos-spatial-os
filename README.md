...
## Cardápios e sons (build 36)

- Os cartões da rotina exibem os alimentos, quantidades e substituições do plano importado. O vínculo usa a chave estável da refeição: alterar título ou horário não perde o cardápio. A tarefa de acordar também aceita a orientação correspondente. As informações do documento são transcritas, sem prescrição ou recálculo nutricional.
- O plano completo pode ser aberto pelos cartões, pelo projeto GYM ou pelas configurações. Os horários vêm das tarefas existentes; a importação não cria tarefas nem modifica conclusões.
- `user_resources` guarda os recursos importados em linhas separadas do histórico. RLS permite somente SELECT da própria conta autenticada. Clientes não podem sobrescrever nem excluir importações. O PDF, alimentos, quantidades e arquivos de som não entram no pacote público ou nas migrações.
- Os nove clipes curtos foram convertidos para MP3 e ficam no registro privado, com conteúdo base64 limitado a esses efeitos pequenos. Depois de carregar, uma cópia local vinculada à conta permite uso sem rede. Sair da conta remove essa cópia; o cache do service worker continua reservado ao código público. Os recursos importados são separados do backup JSON das tarefas.
